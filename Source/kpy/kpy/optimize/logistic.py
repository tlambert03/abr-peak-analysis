...
def evaluate(x, a, b, c, d):
    return a / (1 + np.exp(c)*np.exp(-b*x)) + d;

def inverse(y, a, b, c, d):
    if a/(y-d) <= 0:
        return float('NaN')
    
    return -1/b * (np.log(a/(y-d) - 1) - c)
    
def initial_guess(x, y):
    a = np.max(y)
    xh = np.mean(x)
    b = (a - np.min(y)) / (np.max(x) - np.min(x)) * 4 / a
    c = b * xh  
    return np.array([b, c])
...
def fit(x,y): 
    ...
    param = _compute_params(x, y, *P)
    yfit = evaluate(x, *param);
    
    stats = fitstats(y, yfit, 4)
    
    return collections.namedtuple('result', 'param,x,yfit,stats')(param,x,yfit,stats)
